class Robot : Player
{
    ...
    private readonly Dictionary<Board, Move> possiblePositions;

    public Robot() : this(0, new Board(), new Dictionary<Board, Move>(), "robot") { }

    public Robot(int _side, Board _chessBoard, Dictionary<Board, Move> _positions, string _name) : base(_side, _chessBoard, _name)
    {
        side = _side;
    ...
    {
        if (possiblePositions.ContainsKey(_position)) 
        {
            return new MoveTree(new Node(possiblePositions[_position]));
        }

        Dictionary<int, MoveTree> moveMap = new Dictionary<int, MoveTree>();
        Robot me = new Robot(Side(), _position, new Dictionary<Board, Move>(), "robot1");
        Robot enemy = new Robot(Side() == 1 ? 0 : 1, _position, new Dictionary<Board, Move>(), "robot2");
